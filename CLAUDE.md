...
- **자간** (`segMeasure`): 글자폭 dw=dh*0.46, 글자사이 sep=dh*0.40, 그룹공백 grp=dh*0.75.
  - 2026-09 조정. 레퍼런스를 같은 글자높이(18px)로 맞춰 실측한 결과:
    레퍼런스는 **잉크 폭 11px에 빈칸 8px**, 우리는 **잉크 13px에 빈칸 4px**이었다.
    눈에 보이는 자간이 2배 차이인데 **전체 폭은 6.17 dh vs 6.11 dh로 거의 같았다.**
  - 그래서 간격만 넓히면 각인이 레퍼런스보다 길어진다. **글자를 좁히고(0.55→0.46)
    간격을 넓히는(0.26→0.40) 쌍으로 움직여야 한다.** 한쪽만 만지지 말 것.
