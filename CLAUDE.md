...
- **날짜 포맷**: `'26 9 27` (앞 아포스트로피 + 2자리 연도, 공백 구분) — `fmtDatabackDate()`
  - **월은 0을 안 채운다** (2026-09). 레퍼런스가 `'23 2 16`이라서. 일(day)은 아직 2자리 유지 —
    2월 6일이면 `'26 2 06`이 된다. 일도 떼려면 `fmtDatabackDate`에서 `m[3]`도 같이 처리.
...
- **자간** (`segMeasure`): 글자폭 dw=dh*0.55, 글자사이 sep=dh*0.38, 그룹공백 grp=dh*0.75.
  - 2026-09 조정: 간격만 0.26 → 0.38로 넓혔다. 글자폭은 원래대로 0.55.
  - **측정할 때 함정**: 글리프의 실제 잉크 높이는 `dh`가 아니라 `dh + 두께`다.
    레퍼런스 잉크 높이 18px에 우리 `dh`를 18로 맞추면 우리 쪽이 11% 더 길어져서,
    "레퍼런스 글자가 더 좁다"는 **잘못된 결론**이 나온다(한 번 이렇게 틀려서 0.46까지
    좁혔다가 되돌림). 반드시 **잉크 높이끼리** 맞춰놓고 비교할 것.
  - 잉크 높이를 맞춰 실측한 레퍼런스: 숫자 가로/세로 = **0.67**. 우리 0.55는 0.60,
    0.55+두께 0.112 조합이 0.68로 가장 가깝다.
