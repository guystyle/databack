...
- **로고**:
  - 내장 로고(`BRANDS`)는 **브랜드명을 트래킹 준 굵은 그로테스크로 그린 워드마크 근사치**다.
    공식 트레이드마크 아트워크가 아니다. 캔버스 `letterSpacing` 지원이 들쭉날쭉해서 자간은
    글리프 단위로 직접 그린다(`drawTracked`).
  - 정확한 공식 로고가 필요하면 LOGO 패널에서 **배경 투명 PNG를 업로드** → 워드마크를 대체.
  - 브랜드는 카메라명에서 자동 감지(`detectBrand`, `BRAND_PATTERNS`). 순서가 중요 —
    LUMIX/OM SYSTEM이 부모 브랜드보다 먼저 걸려야 한다. 모델번호만으로 추정하는 규칙은
    오탐이 나서 일부러 안 넣음. 드롭다운에서 수동 지정/끄기 가능.
